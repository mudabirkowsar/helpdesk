...
export default function SignupScreen({ navigation }) {
    ...
    // Navigate to login once token is received
    useEffect(() => {
        if (token) {
    ...
    const handleSignup = () => {
        if (!firstName || !lastName || !email || !password || !confirmPassword) {
            return showErrorMessage(t("signup.error-fill"));
        }
        if (!isValidEmail(email)) {
            return showErrorMessage(t("signup.error-email"));
        }
        if (password !== confirmPassword) {
            return showErrorMessage(t("signup.error-match"));
        }
        ...
        .catch((err) => {
            showErrorMessage(err?.message || t("signup.error-failed"));
        });
    };
    ...
    return (
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.container}>
            <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                {error && <Text style={styles.errorText}>⚠️ {typeof error === 'string' ? error : JSON.stringify(error)}</Text>}

                <TouchableOpacity style={styles.fullButton} onPress={handleSignup} disabled={loading}>
...
const styles = StyleSheet.create({
    ...
    inputContainer: {
        ...
        borderRadius: 12, paddingHorizontal: 15, marginBottom: 20, height: 55,
